...
struct SpaceThumbnailRenderGroup {
    let id: String
    let desktopBounds: CGRect
    let windows: [SpaceThumbnailRenderWindow]
}

struct SpaceThumbnailRenderWindow {
    let frame: CGRect
    let preview: CGImage?
}

enum SpaceThumbnailRenderer {
    static func makeRenderGroups(_ groups: [SpaceSwitcherGroup]) -> [SpaceThumbnailRenderGroup] {
        groups.map { group in
            let windows = group.windows.compactMap { item -> SpaceThumbnailRenderWindow? in
                guard let frame = item.frame?.rect else {
                    return nil
                }
                return SpaceThumbnailRenderWindow(
                    frame: frame,
                    preview: item.preview?.cgImageValue
                )
            }
            return SpaceThumbnailRenderGroup(
                id: group.id,
    ...
    static func render(_ group: SpaceThumbnailRenderGroup) -> CGImage? {
    ...
    private static func drawWindow(
        _ window: SpaceThumbnailRenderWindow,
        desktopBounds: CGRect,
        imageSize: CGSize,
        context: CGContext
    ) {
