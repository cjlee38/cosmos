...
final class KeyboardShortcutResolver {
    func resolve(_ registrations: [KeyboardShortcutRegistration]) throws -> [ResolvedShortcut] {
        ...
        issues.append(contentsOf: spaceTerminalKeyIssues(in: resolved))
    ...
    private func spaceTerminalKeyIssues(in shortcuts: [ResolvedShortcut]) -> [ShortcutValidationIssue] {
        let spaceShortcuts = shortcuts.filter { shortcut in
            guard case .switchSpace? = shortcut.registration.target else {
                return false
            }
            return true
        }
        return duplicateTerminalKeyIssues(in: spaceShortcuts)
    }

    private func duplicateTerminalKeyIssues(
        in spaceShortcuts: [ResolvedShortcut]
    ) -> [ShortcutValidationIssue] {
        Dictionary(grouping: spaceShortcuts, by: \.keystroke.keyCode)
            .values.flatMap { group -> [ShortcutValidationIssue] in
                ...
                return group.map { shortcut in
                    ...
                    return ShortcutValidationIssue(
                        target: shortcut.registration.target,
                        message: "Space selection key is also assigned to \(names.joined(separator: ", "))."
                    )
                }
            }
    }
