...
enum HiddenWindowRecordStartupAction: Equatable {
    case restoreAndAssign(space: SpaceID)
    case assignOnly(space: SpaceID)
    case ignore

    var space: SpaceID? {
        switch self {
        case let .restoreAndAssign(space), let .assignOnly(space):
            space
        case .ignore:
            nil
        }
...
enum HiddenWindowRecordPolicy {
    ...
    static func startupAction(
        ...
        if isFrame(currentFrame, near: record.originalFrame) {
            return .assignOnly(space: record.space)
        }
