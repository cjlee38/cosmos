...
final class SpaceDisplayTopologyTests: SpaceControllerTestCase {
    ...
    func testDisconnectedSpaceFallsBackToMainAndReturnsHomeWhenDisplayReconnects() throws {
        ...
        XCTAssertEqual(controller.visibleSpaces, ["1"])
        XCTAssertFalse(controller.isHiddenBySpace(100))
        XCTAssertTrue(controller.isHiddenBySpace(200))
        XCTAssertEqual(controller.spaceFrame(for: 200), .frame(x: 100, y: 100, width: 300, height: 200))
        XCTAssertEqual(configuredMonitorSlot(for: "A", in: controller), 2)
    ...
    func testCurrentSecondarySpaceStaysVisibleOnMainWhileItsDisplayIsDisconnected() throws {
        let displayProvider = twoDisplayProvider()
        let store = InMemorySpaceConfigStore()
        try store.save(configWithSecondarySpace())
        let windowSystem = FakeWindowSystem(windows: [
        ...
        XCTAssertEqual(controller.visibleSpaces, ["A"])
        XCTAssertTrue(controller.isHiddenBySpace(100))
        XCTAssertFalse(controller.isHiddenBySpace(200))
        XCTAssertEqual(windowSystem.frames[200], .frame(x: 100, y: 100))
    }

    func testMainDisplaySwapMovesSpaceGroupsWithTheirConfiguredRoles() throws {
    ...
    func testCombinedDisplayAndFocusChangeMigratesFrameAndFollowsFocusedSpace() throws {
        let displayProvider = twoDisplayProvider()
        let store = InMemorySpaceConfigStore()
        try store.save(configWithSecondarySpace())
        let windowSystem = FakeWindowSystem(windows: [
